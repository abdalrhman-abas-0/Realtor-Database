...
FROM 'D:\Documents\Data Analysis\projects\realtor_sql\output files\austin neighborhoods.csv' DELIMITER ',' CSV HEADER;
...
FROM 'D:\Documents\Data Analysis\projects\realtor_sql\output files\tax history all.csv' DELIMITER ',' CSV HEADER;
...
FROM 'D:\Documents\Data Analysis\projects\realtor_sql\output files\price history all.csv' DELIMITER ',' CSV HEADER;
...
FROM 'D:\Documents\Data Analysis\projects\realtor_sql\output files\schools all.csv' DELIMITER ',' CSV HEADER;
...
CREATE TABLE school_original AS TABLE schools;

CREATE TABLE schooling AS TABLE schools;

ALTER TABLE schooling DROP COLUMN student_teacher_ratio,
	DROP COLUMN rating,
...
SELECT REGEXP_REPLACE(LTRIM(replace(price, ',', ''), '$'), '/$/d+k', '')::int AS price_,
	...
	REPLACE(parameter, 'sqf', '')::int AS parameter_sqft,TO_JSONB(property_details) AS property_details_,
	
	CASE
		WHEN lot_area LIKE '%sqft%' 
			THEN REPLACE(REPLACE(REPLACE(lot_area, ',', ''),' lot',''),'sqft','')::float
		WHEN lot_area LIKE '%acre%'  
			THEN ROUND(REPLACE(REPLACE(REPLACE(lot_area, ',', ''),' lot',''),'acre','')::float * 43560)
        ELSE NULL
	END AS lot_area_sqf,
	-- CASE
	-- 	WHEN lot_area ~ '/d+sqft' 
	-- 		THEN REPLACE(REPLACE(NULLIF(REPLACE(lot_area, ',', ''), 'without a lot!'),'lot',''),'sqft','')::float
	-- 	WHEN lot_area ~ '/d+acre' 
	-- 		THEN ROUND(REPLACE(REPLACE(NULLIF(REPLACE(lot_area, ',', ''), 'without a lot!'),'lot',''),'acre','')::float * 43560)
	-- END AS lot_area_sqf,
	*
FROM houses;
...
ALTER TABLE houses_copy DROP COLUMN price,
	DROP COLUMN address,
...
ALTER COLUMN total_assessments TYPE int USING total_assessments::integer;
...
ALTER TABLE tax_copy
ALTER COLUMN land TYPE int USING land::integer;
...
ALTER TABLE tax_copy
ALTER COLUMN additions TYPE int USING additions::integer;
...
ALTER TABLE tax_copy
ALTER COLUMN taxes TYPE int USING taxes::integer;
...
ALTER TABLE tax_copy DROP COLUMN added_to,
	DROP COLUMN equals;
...
ALTER TABLE price_history
ALTER COLUMN price_sqft TYPE int USING price_sqft::integer;
...
ALTER TABLE schooling
ALTER COLUMN school_id TYPE int USING school_id::integer;

UPDATE schooling
SET education_levels = REPLACE(
		REPLACE(REPLACE(education_levels, '''', '"'), '[', '{'),
		']',
		'}'
	);
	
ALTER TABLE schooling
ALTER COLUMN education_levels TYPE text [] USING education_levels::text [];

UPDATE SCHOOLING
SET grades = REPLACE(
		REPLACE(REPLACE(grades, '''', '"'), '[', '{'),
		']',
		'}'
	);
	
ALTER TABLE schooling
ALTER COLUMN grades TYPE text [] USING grades::text [];
...
CREATE INDEX idx_home_school ON schooling(listing_page);

CREATE INDEX idx_home_taxes ON tax_history(profile);

CREATE INDEX idx_home_history ON price_history(profile);
...
FROM houses
	LEFT JOIN (
		SELECT DISTINCT *
		FROM tax_history
	) th ON th.profile = houses.listing_page
